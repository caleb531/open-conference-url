...
def test_5mins_before(out, event_dicts):
    ...
    assert feedback["items"][0]["title"] == "My Meeting"
    assert feedback["items"][0]["subtitle"] == "8:00am"
    assert feedback["items"][0]["text"]["copy"] == event_dicts[0]["location"]
    assert feedback["items"][0]["text"]["largetype"] == event_dicts[0]["location"]
...
def test_before_window(out, event_dicts):
    ...
    list_events.main()
    feedback = json.loads(out.getvalue())
    assert feedback["items"][0]["title"] == "No Upcoming Meetings"
    assert feedback["items"][1]["title"] == "My Meeting"
    assert feedback["items"][1]["text"]["copy"] == event_dicts[0]["location"]
    assert feedback["items"][1]["text"]["largetype"] == event_dicts[0]["location"]
    assert len(feedback["items"]) == 2
...
def test_all_day_standalone(out, event_dicts):
    ...
    list_events.main()
    feedback = json.loads(out.getvalue())
    assert feedback["items"][0]["title"] == "All-Day Conference"
    assert feedback["items"][0]["subtitle"] == "All-Day"
    assert feedback["items"][0]["text"]["copy"] == event_dicts[0]["location"]
    assert feedback["items"][0]["text"]["largetype"] == event_dicts[0]["location"]
    assert len(feedback["items"]) == 1
...
def test_all_day_with_past(out, event_dicts):
    ...
    list_events.main()
    feedback = json.loads(out.getvalue())
    assert feedback["items"][0]["title"] == "My Meeting 1"
    assert feedback["items"][0]["subtitle"] == "6:00am"
    assert feedback["items"][0]["text"]["copy"] == event_dicts[0]["location"]
    assert feedback["items"][0]["text"]["largetype"] == event_dicts[0]["location"]
    assert feedback["items"][1]["title"] == "All-Day Conference"
    assert feedback["items"][1]["subtitle"] == "All-Day"
    assert feedback["items"][1]["text"]["copy"] == event_dicts[1]["location"]
    assert feedback["items"][1]["text"]["largetype"] == event_dicts[1]["location"]
    assert len(feedback["items"]) == 2
...
def test_all_day_mixed(out, event_dicts):
    ...
    list_events.main()
    feedback = json.loads(out.getvalue())
    assert feedback["items"][0]["title"] == "Morning Scrum"
    assert feedback["items"][0]["subtitle"] == "8:00am"
    assert feedback["items"][0]["text"]["copy"] == event_dicts[1]["location"]
    assert feedback["items"][0]["text"]["largetype"] == event_dicts[1]["location"]
    assert feedback["items"][1]["title"] == "All-Day Conference"
    assert feedback["items"][1]["subtitle"] == "All-Day"
    assert feedback["items"][1]["text"]["copy"] == event_dicts[0]["location"]
    assert feedback["items"][1]["text"]["largetype"] == event_dicts[0]["location"]
...
def test_multiple_meetings_at_once(out, event_dicts):
    ...
    assert feedback["items"][0]["text"]["copy"] == event_dicts[0]["location"]
    assert feedback["items"][0]["text"]["largetype"] == event_dicts[0]["location"]
    assert feedback["items"][1]["title"] == "My Meeting 2"
    assert feedback["items"][1]["subtitle"] == "8:15am"
    assert feedback["items"][1]["text"]["copy"] == event_dicts[1]["location"]
    assert feedback["items"][1]["text"]["largetype"] == event_dicts[1]["location"]
    assert len(feedback["items"]) == 2
...
def test_past_meetings_only(out, event_dicts):
    ...
    assert feedback["items"][0]["title"] == "No Upcoming Meetings"
    assert feedback["items"][1]["title"] == "My Meeting 2"
    assert feedback["items"][1]["subtitle"] == "8:00am"
    assert feedback["items"][1]["text"]["copy"] == event_dicts[1]["location"]
    assert feedback["items"][1]["text"]["largetype"] == event_dicts[1]["location"]
    assert feedback["items"][2]["title"] == "My Meeting 1"
    assert feedback["items"][2]["subtitle"] == "6:00am"
    assert feedback["items"][2]["text"]["copy"] == event_dicts[0]["location"]
    assert feedback["items"][2]["text"]["largetype"] == event_dicts[0]["location"]
...
def test_past_meetings_with_upcoming(out, event_dicts):
    ...
    list_events.main()
    feedback = json.loads(out.getvalue())
    assert feedback["items"][0]["title"] == "My Meeting 3"
    assert feedback["items"][0]["subtitle"] == "8:00am"
    assert feedback["items"][0]["text"]["copy"] == event_dicts[2]["location"]
    assert feedback["items"][0]["text"]["largetype"] == event_dicts[2]["location"]
    assert feedback["items"][1]["title"] == "My Meeting 2"
    assert feedback["items"][1]["subtitle"] == "7:15am"
    assert feedback["items"][1]["text"]["copy"] == event_dicts[1]["location"]
    assert feedback["items"][1]["text"]["largetype"] == event_dicts[1]["location"]
    assert len(feedback["items"]) == 2
...
@freeze_time("2022-10-16 07:55:00")
@redirect_stdout
def test_multiple_past_meetings_with_upcoming(out, event_dicts):
...
@redirect_stdout
def test_no_events_for_today(out, _event_dicts):
    """Should display no meetings if there are no events for today"""
    list_events.main()
    feedback = json.loads(out.getvalue())
    assert feedback["items"][0]["title"] == "No Results"
    assert len(feedback["items"]) == 1
...
def test_24_hour(out, event_dicts):
    ...
    feedback = json.loads(out.getvalue())
    assert feedback["items"][0]["title"] == "My Meeting"
    assert feedback["items"][0]["subtitle"] == "13:00"
    assert feedback["items"][0]["text"]["copy"] == event_dicts[0]["location"]
    assert feedback["items"][0]["text"]["largetype"] == event_dicts[0]["location"]
    assert len(feedback["items"]) == 1
